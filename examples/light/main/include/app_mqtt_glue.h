...
#define __MQTT_QOS0    0
#define __MQTT_QOS1    1
 ...
 *
 * @return Pointer to \ref esp_mqtt_conn_params_t on success.
 * @return NULL on failure.
 */
typedef esp_mqtt_conn_params_t *(*esp_mqtt_get_conn_params_t)(void);
 ...
 * \ref esp_mqtt_get_conn_params call if registered.
 *
 * @return ESP_OK on success.
 * @return error in case of any error.
 */
typedef esp_err_t (*esp_mqtt_init_t)(esp_mqtt_conn_params_t *conn_params);

/** MQTT Deinit function prototype
 *
 * Call this function after MQTT has disconnected.
 */
typedef void (*esp_mqtt_deinit_t)(void);
 ...
 * @return error in case of any error.
 */
typedef esp_err_t (*esp_mqtt_connect_t)(void);
 ...
 * @return error in case of any error.
 */
typedef esp_err_t (*esp_mqtt_disconnect_t)(void);
 ...
 * @return error in case of any error.
 */
typedef esp_err_t (*esp_mqtt_publish_t)(const char *topic, void *data, size_t data_len, uint8_t qos, int *msg_id);
 ...
 * @return error in case of any error.
 */
typedef esp_err_t (*esp_mqtt_subscribe_t)(const char *topic, esp_mqtt_subscribe_cb_t cb, uint8_t qos, void *priv_data);
 ...
 * @return error in case of any error.
 */
typedef esp_err_t (*esp_mqtt_unsubscribe_t)(const char *topic);
...
typedef struct {
    ...
    esp_mqtt_get_conn_params_t get_conn_params;
    /** Pointer to MQTT Init function. */
    esp_mqtt_init_t init;
    /** Pointer to MQTT Deinit function. */
    esp_mqtt_deinit_t deinit;
    /** Pointer to MQTT Connect function. */
    esp_mqtt_connect_t connect;
    /** Pointer to MQTQ Disconnect function */
    esp_mqtt_disconnect_t disconnect;
    /** Pointer to MQTT Publish function */
    esp_mqtt_publish_t publish;
    /** Pointer to MQTT Subscribe function */
    esp_mqtt_subscribe_t subscribe;
    /** Pointer to MQTT Unsubscribe function */
    esp_mqtt_unsubscribe_t unsubscribe;
} esp_mqtt_config_t;
