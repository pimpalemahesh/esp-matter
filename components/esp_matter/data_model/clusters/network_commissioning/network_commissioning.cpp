...
{
    return esp_matter::attribute::create(cluster, MaxNetworks::Id, ATTRIBUTE_FLAG_MANAGED_INTERNALLY, esp_matter_uint8(value));
}
...
attribute_t *create_scan_max_time_seconds(cluster_t *cluster, uint8_t value)
{
    ...
    return esp_matter::attribute::create(cluster, ScanMaxTimeSeconds::Id, ATTRIBUTE_FLAG_MANAGED_INTERNALLY, esp_matter_uint8(value));
}
...
attribute_t *create_connect_max_time_seconds(cluster_t *cluster, uint8_t value)
{
    uint32_t feature_map = get_feature_map_value(cluster);
    VerifyOrReturnValue(((feature_map & feature::wi_fi_network_interface::get_id()) || (feature_map & feature::thread_network_interface::get_id())), NULL);
    return esp_matter::attribute::create(cluster, ConnectMaxTimeSeconds::Id, ATTRIBUTE_FLAG_MANAGED_INTERNALLY, esp_matter_uint8(value));
}

attribute_t *create_interface_enabled(cluster_t *cluster, bool value)
{
    return esp_matter::attribute::create(cluster, InterfaceEnabled::Id, ATTRIBUTE_FLAG_WRITABLE | ATTRIBUTE_FLAG_MANAGED_INTERNALLY | ATTRIBUTE_FLAG_NONVOLATILE, esp_matter_bool(value));
}

attribute_t *create_last_networking_status(cluster_t *cluster, nullable<uint8_t> value)
{
    return esp_matter::attribute::create(cluster, LastNetworkingStatus::Id, ATTRIBUTE_FLAG_MANAGED_INTERNALLY | ATTRIBUTE_FLAG_NULLABLE, esp_matter_nullable_enum8(value));
}

attribute_t *create_last_network_id(cluster_t *cluster, uint8_t *value, uint16_t length)
{
    return esp_matter::attribute::create(cluster, LastNetworkID::Id, ATTRIBUTE_FLAG_MANAGED_INTERNALLY | ATTRIBUTE_FLAG_NULLABLE, esp_matter_octet_str(value, length));
}

attribute_t *create_last_connect_error_value(cluster_t *cluster, nullable<int32_t> value)
{
    return esp_matter::attribute::create(cluster, LastConnectErrorValue::Id, ATTRIBUTE_FLAG_MANAGED_INTERNALLY | ATTRIBUTE_FLAG_NULLABLE, esp_matter_nullable_int32(value));
}
...
attribute_t *create_supported_thread_features(cluster_t *cluster, uint16_t value)
{
    ...
    return esp_matter::attribute::create(cluster, SupportedThreadFeatures::Id, ATTRIBUTE_FLAG_MANAGED_INTERNALLY, esp_matter_bitmap16(value));
}
...
cluster_t *create(endpoint_t *endpoint, config_t *config, uint8_t flags)
{
    ...
    if (flags & CLUSTER_FLAG_SERVER) {
        ...
        attribute::create_max_networks(cluster, 1);
        attribute::create_networks(cluster, NULL, 0, 0);
        attribute::create_interface_enabled(cluster, true);
        ...
        if (feature_map & feature::wi_fi_network_interface::get_id()) {
            VerifyOrReturnValue(feature::wi_fi_network_interface::add(cluster) == ESP_OK, ABORT_CLUSTER_CREATE(cluster));
        }
        if (feature_map & feature::thread_network_interface::get_id()) {
            VerifyOrReturnValue(feature::thread_network_interface::add(cluster) == ESP_OK, ABORT_CLUSTER_CREATE(cluster));
        }
