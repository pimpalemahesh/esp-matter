...
endpoint_t *create(node_t *node, config_t *config, uint8_t flags, void *priv_data)
{
    ...
    VerifyOrReturnValue(endpoint != nullptr, NULL, ESP_LOGE("on_off_plug_in_unit", "Failed to create endpoint"));

    cluster_t *descriptor_cluster = descriptor::create(endpoint, &(config->descriptor), CLUSTER_FLAG_SERVER);
    VerifyOrReturnValue(descriptor_cluster != nullptr, NULL, ESP_LOGE("on_off_plug_in_unit", "Failed to create descriptor cluster"));

    VerifyOrReturnValue(add(endpoint, config) == ESP_OK, NULL, ESP_LOGE("on_off_plug_in_unit", "Failed to add device type"));
    return endpoint;
}

esp_err_t add(endpoint_t *endpoint, config_t *config)
{
    esp_err_t err = add_device_type(endpoint, ESP_MATTER_ON_OFF_PLUG_IN_UNIT_DEVICE_TYPE_ID, ESP_MATTER_ON_OFF_PLUG_IN_UNIT_DEVICE_TYPE_VERSION);
