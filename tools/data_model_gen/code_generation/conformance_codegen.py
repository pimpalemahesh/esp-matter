...
def filter_operand(operands: List[Expr]) -> List[Expr]:
    ...
    for op in operands:
        str_expr = op()
        if str_expr is not None and "true" not in str_expr and "false" not in str_expr:
            filtered_operands.append(op)
...
def parse_expr(obj: dict) -> Expr:
    ...
    elif ATTRIBUTE_TAG in obj:
        return AttributeExpr(obj[ATTRIBUTE_TAG])

    elif FEATURE_TAG in obj:
        return FeatureExpr(obj[FEATURE_TAG])

    elif COMMAND_TAG in obj:
        return CommandExpr(obj[COMMAND_TAG], obj[COMMAND_FLAG_TAG])

    elif NOT_TERM in obj:
        return NotExpr(parse_expr(obj[NOT_TERM]))

    elif AND_TERM in obj:
        return AndExpr([parse_expr(x) for x in obj[AND_TERM]])

    elif OR_TERM in obj:
        return OrExpr([parse_expr(x) for x in obj[OR_TERM]])

    elif GREATER_TERM in obj:
...
def parse_choice(conformance: Dict[str, Any]) -> Optional[Choice]:
    ...
    nested_conformance = conformance.get(KEY_CONDITION, {})
...
def get_conformance_type(type: str) -> ConformanceDecision:
    ...
    if type == MANDATORY_CONFORM:
        return ConformanceDecision.MANDATORY
    elif type == OPTIONAL_CONFORM:
        return ConformanceDecision.OPTIONAL
    elif type == OTHERWISE_CONFORM:
        return ConformanceDecision.OTHERWISE
    elif type == DEPRECATE_CONFORM:
        return ConformanceDecision.DEPRECATE
    elif type == DISALLOWED_CONFORM:
        return ConformanceDecision.DISALLOWED
    elif type == PROVISIONAL_CONFORM:
...
class Conformance(BaseConformance, Expr):
    ...
    def get_optional_condition(self) -> Expr:
        ...
        if self.type == ConformanceDecision.OPTIONAL:
            condition = self.conformance.get(KEY_CONDITION, {})
            if not condition:
                return None
            return OptionalExpr(parse_expr(condition))
        elif self.type == ConformanceDecision.OTHERWISE:
            optional_condition = self.conformance.get(KEY_CONDITION, {}).get(
                OPTIONAL_CONFORM
            )
            operands = []
    ...
    def _generate_condition(self, conformance: Dict[str, Any]) -> Expr:
        ...
        :raises: ValueError if the conformance type is invalid.
        """
        if not conformance:
            return None
        condition = conformance.get(KEY_CONDITION)
        if not condition:
            return None
        ...
        elif self.type == ConformanceDecision.DEPRECATE:
            return DeprecateExpr(parse_expr(condition))
        else:
            raise ValueError(f"Unknown conformance type: {self.type}")
        ...
        if conformance and self.type == ConformanceDecision.OTHERWISE:
            ...
            if condition and isinstance(condition, dict):
                ...
                if (
                    ...
                    and FEATURE_TAG in mandatory
                ):
                    self.mandatory_parent = mandatory[FEATURE_TAG]
        elif conformance and self.type == ConformanceDecision.OPTIONAL:
            condition = conformance.get(KEY_CONDITION)
            if condition and isinstance(condition, dict):
                self.optional_parent = condition.get(FEATURE_TAG)

        elif conformance and self.type == ConformanceDecision.MANDATORY:
            condition = conformance.get(KEY_CONDITION)
            if condition and isinstance(condition, dict):
                self.mandatory_parent = condition.get(FEATURE_TAG)
