...
from utils.exceptions import CodeGenerationError, ConfigurationError
...
def get_template(template_name):
    ...
    if template is None:
        raise CodeGenerationError(
            ...
            suggestion="Check that template files exist in the templates directory.",
        )
    return template
...
def generate_device_files(json_path, output_dir, clusters: List[Cluster]):
    ...
    header_file_path = os.path.join(output_dir, "device_types", "all_device_types.h")
    ...
    try:
        ...
        if cluster_json_path:
            if not os.path.exists(cluster_json_path):
                raise ConfigurationError(
                    "Cluster JSON file not found",
                    file_path=cluster_json_path,
                    context="code_generator.main",
                    suggestion="Run XML parsing step first to generate cluster JSON, or provide a valid path.",
                )
            clusters = generate_cluster_files(cluster_json_path, output_dir)
        if device_json_path:
            if not os.path.exists(device_json_path):
                raise ConfigurationError(
                    "Device JSON file not found",
                    file_path=device_json_path,
                    context="code_generator.main",
                    suggestion="Run XML parsing step first to generate device JSON, or provide a valid path.",
                )
            generate_device_files(device_json_path, output_dir, clusters)

        if not cluster_json_path and not device_json_path:
            raise ConfigurationError(
                "Either cluster JSON path or device JSON path must be provided",
                context="code_generator.main",
                suggestion="Check --help for available options.",
            )
