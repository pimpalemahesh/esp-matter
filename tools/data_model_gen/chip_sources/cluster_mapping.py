...
from chip_sources.server_files_config import (
    local_mappings,
    parser,
    PLUGIN_CB_PATTERN,
    DELEGATE_METHODS,
...
def generate_delegate_cluster_mapping(root_dir, delegate_cluster_json_file_path):
    ...
    if write_to_file(delegate_cluster_json_file_path, delegate_server_files, "json"):
        logger.info(
            f"Successfully written to delegate clusters to {delegate_cluster_json_file_path}"
        )
        return True
    return False
...
def generated_plugin_init_cb_cluster_mapping(
    ...
    if write_to_file(plugin_init_cb_cluster_json_file_path, cluster_names, "json"):
        logger.info(
            f"Successfully written to plugin init callback clusters to {plugin_init_cb_cluster_json_file_path}"
        )
        return True
    return False
