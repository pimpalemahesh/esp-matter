...
def get_attributes_from_server_files(root_dir, json_file_path) -> bool:
    ...
    if not os.path.isdir(root_dir):
        logger.warn(f"Directory {root_dir} does not exist")
        return False
...
def get_internally_managed_attributes(
    ...
    if server_success:
        try:
            ...
            logger.debug(f"Loaded {len(server_attributes)} clusters from server files")
        except Exception as e:
    ...
    try:
        ...
        logger.info(f"Successfully generated combined attributes at {output_json_path}")
    except Exception as e:
        logger.error(f"Error writing combined attributes: {e}")
        return False
