...
class Attribute(BaseAttribute):
    ...
    def __init__(
        ...
        self.is_nullable = (
            ...
            and safe_get_attr(safe_get_attr(self, "quality"), "nullable").lower()
            == "true"
            else False
        )
    ...
    def get_flag(self):
        ...
        if safe_get_attr(self, "access"):
            if safe_get_attr(safe_get_attr(self, "access"), "write") and (
                safe_get_attr(safe_get_attr(self, "access"), "write").lower() == "true"
                or safe_get_attr(safe_get_attr(self, "access"), "write").lower()
                == "optional"
            ):
        ...
        if safe_get_attr(self, "quality"):
            if (
                safe_get_attr(safe_get_attr(self, "quality"), "nullable")
                and safe_get_attr(safe_get_attr(self, "quality"), "nullable").lower()
                == "true"
            ):
                flags.append(self.AttributeFlags.ATTRIBUTE_FLAG_NULLABLE)
            if (
                safe_get_attr(safe_get_attr(self, "quality"), "persistence")
                and safe_get_attr(safe_get_attr(self, "quality"), "persistence").lower()
                == "nonvolatile"
            ):
    ...
    def _convert_default_values(self):
        ...
        if self.type == "bool":
            ...
            return (
                "true"
                if self.default_value.lower() == "true"
                or self.default_value.lower() == "1"
                else "false"
            )

        if self.type == "string" or self.type == "octstr":
            if (
                safe_get_attr(self, "constraint") is not None
                and safe_get_attr(safe_get_attr(self, "constraint"), "type")
                == "maxLength"
                and safe_get_attr(safe_get_attr(self, "constraint"), "value")
                is not None
            ):
                return int(safe_get_attr(safe_get_attr(self, "constraint"), "value"))
        ...
        if self.type == "list":
            if (
                safe_get_attr(self, "constraint") is not None
                and safe_get_attr(safe_get_attr(self, "constraint"), "value")
                and safe_get_attr(safe_get_attr(self, "constraint"), "value").isdigit()
            ):
                return int(safe_get_attr(safe_get_attr(self, "constraint"), "value"))
            if self.default_value is not None and self.default_value in [
                "null",
        ...
        if (
            self.default_value is not None and "°" in self.default_value
        ):  # for temperatures
...
class Cluster(BaseCluster):
    ...
    def get_plugin_server_init_callback(self):
        ...
        if not self.plugin_init_cb_available or should_skip_plugin_callback(self.esp_name):
            logger.debug(f"No plugin server init callback found for {self.name}")
            return None
    ...
    def get_delegate_init_callback(self):
        ...
        if self.delegate_init_callback_available and not should_skip_delegate_callback(
            ...
            logger.debug(f"Delegate init callback found for {self.name}")
    ...
    def get_mandatory_attributes(self):
        ...
        for attribute in self.attributes:
            conformance = safe_get_attr(attribute, "conformance")
            if attribute.is_mandatory and conformance is not None:
                condition = (
                    conformance.get("condition")
        ...
        if len(mandatory_attributes) > 0:
            mandatory_attributes.sort(key=lambda x: (int(x.get_id(), 16), x.name))
    ...
    def get_mandatory_commands(self):
        ...
        for command in self.commands:
            conformance = safe_get_attr(command, "conformance")
            if command.is_mandatory and conformance is not None:
                condition = (
                    conformance.get("condition")
                    if isinstance(conformance, dict)
                    else None
        ...
        if len(mandatory_commands) > 0:
            mandatory_commands.sort(key=lambda x: (int(x.get_id(), 16), x.name))
    ...
    def get_mandatory_events(self):
        ...
        for event in self.events:
            conformance = safe_get_attr(event, "conformance")
            if event.is_mandatory and conformance is not None:
                condition = (
                    conformance.get("condition")
                    if isinstance(conformance, dict)
                    else None
